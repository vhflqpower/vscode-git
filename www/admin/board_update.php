...
if($bo_table=='info'){			
	...
	goto_url("./board_view.php?wr_id=$wr_id&part=$part&bo_table=$bo_table");
	...
	$query ="update psj_board set
		...
		sql_query(" update {$g5['board_table']} set bo_notice = '{$bo_notice}' where bo_table = '{$bo_table}' ");
			...
			$url = "board_view.php?wr_id=$wr_id&part=$part&bo_table=$bo_table&page=$page";

	if($member[mb_id]=='admin'){
			
			mailege_save($bo_table,$mb_id,$url,$wr_status,$info_level,$wr_confirm,$info_point);  // 마일리지부여
		...
		goto_url("./board_view.php?wr_id=$wr_id&part=$part&bo_table=$bo_table&page=$page");


	}else if($oper=='del' && $member[mb_id]=='admin'){
	...
	$sql3 = " select wr_id,bf_datetime,bf_file from psj_board_img where bo_table = '$bo_table' and wr_id = '$wr_id'"; 
	 $result3 = sql_query($sql3); 
		...
		goto_url("./board_list.php?part=$part&bo_table=$bo_table");
