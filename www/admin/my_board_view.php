<?
	...
	if($wr_id){
			...
			// 조회 COUNT
			sql_query("update `psj_board` set wr_hit = wr_hit+1 where wr_id = '$wr_id' ");
			// 조회자 로그
		if($bo_table=='info'){
			sql_query("insert into psj_view_log values('','info','$wr_id','$member[mb_id]',Now())");
			// 조회자 로그 카운트update
	...
	<? } ?>
          ...
          <div class="table-responsive">
			<form name="frm" method="post" action="./board_update.php" onSubmit="return saveSubmit(f)" enctype="multipart/form-data" >
				...
				<input type="hidden"  id="bo_table" value="<?=$view['bo_table']?>" />
		...
		<? if($view[mb_id]==$member[mb_id] || $member[mb_id]=='admin'){ ?>	
				...
				<button type="button" class="btn btn-default btn-sm"  onclick="location.href='./my_board.php?part=<?=$part?>&bo_table=<?=$bo_table?>&page=<?=$page?>'">
				  <span class="glyphicon glyphicon-list-alt" aria-hidden="true"></span>목 록
				</button>
		<? if($view[mb_id]==$member[mb_id] &&  $_GET[part] == 'myinfo'){ ?>		
				<button type="button" class="btn btn-default btn-sm"  onclick="location.href='./my_board_write.php?wr_id=<?=$view[wr_id]?>&part=<?=$part?>&bo_table=<?=$bo_table?>&page=<?=$page?>'">
...
<? if($bo_table =='info' || $bo_table =='bugreport'){ ?>
			<form name="form_comm" method="post" action="./board_update.php">
...
?>
	...
	<form name="fdel" method="post" action="./my_board_update.php">
...
*/
	...
	var url = './board_update.php'//url 수정;
		$.ajax({
			url:url,
			data: postData,
			type:'post',
...
<? if($part=='myinfo' || $part=='info' || $part=='myopeninfo'){ ?> 
