...
//	include_once("./dbconfig.php");
	...
	$i = 0;
		if (!$_GET['search_value'])
		{
			...
			for ($k=0; $k<count($arr_notice); $k++)
			{
				...
				$row = sql_fetch(" select * from psj_board_data where wr_id = '$arr_notice[$k]' ");
	...
	$page = $_GET['page'];
	$rowcnt = sql_query("SELECT count(*) as cnt from psj_board_data  where 1=1 ".$WHERE);
	$cntsql = sql_fetch_array($rowcnt);

	$sql = "SELECT * from psj_board_data where  1=1 ".$WHERE." ORDER BY wr_id DESC";
	$paginationlink = "/admin/ajax/ajax_data_row_data.php?bo_table=$bo_table&page=";				
	...
	<col width="5%">
	  ...
	  <tbody >
        ...
        for ($i=0; $i<count($list); $i++) {
         ?>
		 <tr>
			 <td><div style="text-align:center"><font color=blue><?=$row['wr_id']?></font></div></td>
			 <td >
			<?//php echo $list[$i]['gubun']?>공지
			 </td>
			 <td id=item_<?=$row['wr_id']?>><strong><a href="./data_view.php?wr_id=<?=$list[$i]['wr_id']?>&part=<?=$part?>&bo_table=<?=$bo_table?>"><?php echo $list[$i]['subject'] ?></a></strong>
...
?>
		 ...
		 <tr>
			 <td><div style="text-align:center"><font color=blue><?=$wr_id?></font></div></td>
			 <td ><!-- <div align=center> -->
			...
			<? }else{ ?>
			 ...
			 <td id=item_<?=$row['wr_id']?>><strong><a href="./data_view.php?wr_id=<?=$wr_id?>&part=<?=$part?>&bo_table=<?=$bo_table?>&page=<?=$page?>"><?=$wr_subject?></a></strong>
			<? if($com_cnt > 0){ ?><span class="badge"><?=$com_cnt?></span><? } ?>
			 <?=$list['icon_new']?>
...
<?
		...
		echo "<button type='button' class='btn btn-primary btn-sm' onclick=\"location.href='./data_write.php?part=$part&bo_table=$bo_table'\">
