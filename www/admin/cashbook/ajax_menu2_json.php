<?php
	...
	$result = mysql_query("SELECT COUNT(*) AS count FROM TB_CM_MENU_SUB  where me_part = '2' and p_code = '$id'");
	...
	$sql = "SELECT * from TB_CM_MENU_SUB   where me_part = '2' and p_code = '$id' ORDER BY $sidx $sord LIMIT $start , $limit";
	...
	while($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
		$responce->rows[$i]['id']=$row['me_code'];
		$responce->rows[$i]['cell']=array($row['me_code'],$row['me_name'],$row['p_code']); 
		$i++;
	}       
