...
//		odata : ['equal', 'not equal', 'less', 'less or equal','greater','greater or equal', 'begins with','does not begin with','is in','is not in','ends with','does not end with','contains','does not contain'],
		odata : ['같다', '같지 않다', '적다', '같거나 적다','크다','같거나 크다', '단어로 시작','단어로 시작 안 함','들어있는 단어','들어있지 않은 단어','단어로 끝남','해당 단어로 끝나지 않음','포함됨','포함되지 않음','비어있음','비어있지않음'],
	...
	edit : {
		...
		msg: {
			...
			nodefined : " 선언되지 않았습니다.!",
			novalue : " 리턴값이 필요합니다!",
			customarray : "사용자 함수는 배열을 반환해야 합니다.!",
			customfcheck : "사용자 함수가 정의되어 있지 않습니다.!"
		},
		modal:false,
		width: 'auto',
		height: 'auto',
		...
		closeOnEscape:true,
		topinfo:"(*)표시된 항목은 필수입니다.",
		afterSubmit:function(response, postdata)
		{
			var ret = eval('(' + response.responseText + ')');
			var success = (ret.flag == 'succ');
...
/*
		,afterComplete:function (response, postdata, formid) {
			...
			var message = ret.message;
			var new_id = ret.id;
			$("#bn_list").jqGrid('setCell',new_id,'permitno_id',new_id);
		}
*/
	},
	view : {
	...
	errors : {
		...
		model : "colNames 와 colModel 길이가 다르면 안됩니다!"
	},
	formatter : {
