...
<? if($bo_table=='info'){ ?>
					...
					<th>분류</th>
					<td>
						<?=$cat1name[$cat1][0]?> > <span style="color:#0000ff"><?=$cat2name[$cat2][0]?></span>
					<span style="float:right;margin-right:20px;"><?echo ($view['wr_confirm']=='Y')?"승인완료":"검토중";?></span>
					</td>
					</tr>
<? } ?>
					<tr>
		...
		<? if($view[mb_id]==$member[mb_id] || $member[mb_id]=='admin'){ ?>	
				...
				<button type="button" class="btn btn-default btn-sm"  onclick="location.href='./board_list.php?part=<?=$part?>&bo_table=<?=$bo_table?>&page=<?=$page?>'">
				  <span class="glyphicon glyphicon-list-alt" aria-hidden="true"></span>목 록
				</button>
		<? if($view[mb_id]==$member[mb_id] || $member[mb_id]=='admin'){ ?>		
				<button type="button" class="btn btn-default btn-sm"  onclick="location.href='./board_write.php?wr_id=<?=$view[wr_id]?>&part=<?=$part?>&bo_table=<?=$bo_table?>&page=<?=$page?>'">
