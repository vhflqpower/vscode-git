...
include_once($g5_path['path'].'/lib/config.php');   // 설정 파일
include_once($g5_path['path'].'/lib/member.class.php');   // 설정 파일
include_once($g5_path['path'].'/lib/ckeditor.lib.php');   // ckeditor
...
if ($_SESSION['ss_mb_id']) { // 로그인중이라면
  ...
  //  $menuAuth = get_member_access($_SESSION['ss_mb_id']);
...
if ($part) {
    ...
    $menu_config = sql_fetch(" select * from psj_menu_config where me_code = '$part' ");
...
// 방문자수의 접속을 남김
include_once(G5_LIB_DIR.'/visit_insert.inc.php');
...
// 페이징
   ...
   function page($page_count,$page, $total_page, $href){
	  $start_page=(int)(($page-1)/$page_count)*$page_count;
      //echo "==>$start_page";
      //echo "==>$start_page";
   //   echo "==>".(($page-1)/$page_count)*$page_count;
      $i=1;
      $prev_page = $start_page-1; 
      if($prev_page < 1) $prev_page = 1;
      if($start_page < 1) $prev_page_str = $prev_page;
      else $prev_page_str = $prev_page+1;
      echo "<span>";
      echo"<a href='$PHP_SELF?page=1$href'>처음</a>";
      echo" &nbsp;<a href='$PHP_SELF?page=$prev_page_str$href'>이전</a>";
      while($i+$start_page<=$total_page&&$i<=$page_count){
         $move_page=$i+$start_page;
         if($page==$move_page) echo" <b>$move_page</b> ";
         else echo" <a href='$PHP_SELF?page=$move_page$href'>$move_page</a> ";
		 
		 
         $i++;
      }
      $next_page = $move_page+1;
      if($next_page > $move_page) $next_page = $move_page;
      //echo "==>$next_page";
      if($next_page == $total_page) $next_page_str = $next_page;
      else $next_page_str = $next_page+1;

      echo" &nbsp;<a href='$PHP_SELF?page=$next_page_str$href'>다음</a>&nbsp; ";
      echo"<a href='$PHP_SELF?page=$total_page$href'>마지막</a>";
      echo "</span>";
   }
