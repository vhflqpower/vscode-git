...
include_once("./_common.php");
	...
	$grid_pager = 'p'.$grid;
	$grid_div = 'div_'.$grid;
	$ajax_url = './ajax_mb_info_data.php';
...
var $grid_div = false; //전역 그리드페이저 변수
 ...
 $grid.jqGrid({
	...
	colNames:['NO','MBID','담당명','HP','EMAIL','업체','보기'],
	colModel:[
		{name:'mb_no',index:'mb_no', width:70, align:"center", editable:true,hidden:false, viewable: false, hidedlg:false,formoptions:{ rowpos:1 }},
		{name:'mb_1',index:'mb_1', width:114,align:"center",editable:true,editoptions:{readonly:false,size:10},formoptions:{ rowpos:2 }},	
		{name:'mb_name',index:'mb_name', width:100,align:"left",editable:true,editoptions:{readonly:false,size:10},formoptions:{ rowpos:3 }},	
		{name:'mb_hp',index:'mb_hp', width:100,align:"left",editable:true,editoptions:{readonly:false,size:10},formoptions:{ rowpos:3 }},	
		{name:'mb_email',index:'mb_email', width:100,align:"left",editable:true,editoptions:{readonly:false,size:10},formoptions:{ rowpos:3 }},
		{name:'company',index:'company', width:100,align:"left",editable:true,editoptions:{readonly:false,size:10},formoptions:{ rowpos:3 }},	
...
/* edittype:'textarea', */
	...
	,gridComplete: function(){
		...
		for(var i=0; i < ids.length;i++){
	 	...
	 	var mb_no = $grid.jqGrid("getRowData", cl).mb_no;	
		var mb_name = $grid.jqGrid("getRowData", cl).mb_name;	



			be = "<a href='#' onclick=\"parentsend('"+mb_no+"')\"><input style='height:22px;width:40px;' type='button' value='적용' ></a>"; 
...
function  parentsend(rowid){
		...
		//rowid  = $grid.jqGrid("getGridParam", "selrow");		
		mb_1 = $grid.jqGrid("getRowData", rowid).mb_1;
		mb_no = $grid.jqGrid("getRowData", rowid).mb_no;
		mb_name = $grid.jqGrid("getRowData", rowid).mb_name;
		mb_hp = $grid.jqGrid("getRowData", rowid).mb_hp;
		mb_email = $grid.jqGrid("getRowData", rowid).mb_email;
		company = $grid.jqGrid("getRowData", rowid).company;

		var part = $("#part").val();

		$("#mb_id_"+part, opener.document).val(rowid);
		$("#co_id_"+part, opener.document).val(mb_1);
		$("#mb_name_"+part, opener.document).val(mb_name);
		$("#mb_hp_"+part, opener.document).val(mb_hp);
		$("#mb_email_"+part, opener.document).val(mb_email);
		$("#company_"+part, opener.document).val(company);
