<?php
    ...
    // AND a.gd_id =  'test0324'
		...
		$sql = "SELECT a.seq,
					   ...
					   c.ta_price,
					   a.is_open 
				FROM tt_tag_cell a,tt_tag_item b,tt_tag c 
...
//  (a.ta_price -((a.ta_price / 100) * b.ti_percent)) as taPrice 
	...
	while($row = sql_fetch_array($result,MYSQL_ASSOC)) {
		...
		$responce->rows[$i]['cell']=array($row['tc_no'],$row['seq'],$row['ti_percent'],$row['ta_price'],$row['is_open']);  // $row['regdate_tag'],$row['ti_percent'],$row['taPrice'],$row['is_open']
		$i++;
	}     
