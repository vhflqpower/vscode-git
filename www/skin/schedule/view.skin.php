...
?>
	...
	<div class="page-header">
      ...
      <div class="row row-offcanvas row-offcanvas-right">
		...
		<?if($member['mb_id'] == 'admin' || $member['mb_id'] == $view['mb_id']){ ?>
...
<table width="100%" border="0"> 
			...
			<?
			$query4 = " 
				SELECT idx,mb_id,mb_name,late_yn,regdate FROM `psj_schedule_add` where board_id ='$board[bo_table]' and board_seq = '$wr_id'";
...
<!-- 댓글 리스트 끝 -->
			...
			<input type="hidden" name="at_date" value="<?=$view['wr_1']?>">
...
//출석체크
	function late_confirm(idx,mb_id) {

		if(confirm("지각처리 하시겠습니까?")) {

	
	 $.ajax({
		url: '../board/schedule_add_server.php',
		type: "POST",
		data:{
			idx:idx,
			mode:'edit',
