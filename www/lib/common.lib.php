...
function alert($msg='', $url='', $error=true, $post=false)
{
    ...
    include_once(G5_BOARD_PATH.'/alert.php');
    exit;
}
...
#  $mode=1 리턴없음 $mode=2 리턴있음

function get_member_access($menu,$part,$mb_id,$mode)
{
    global $g5;


	$row = sql_fetch("select menu_cd,part,mb_id,access from psj_menu_auth where mb_id = TRIM('$mb_id') && menu_cd='$menu' && part = '$part'");
...
// 게시판의 다음글 번호를 얻는다.  //psj 161214
function get_next_no($table,$tb_id)
{
    // 가장 작은 번호를 얻어
    $sql = " select max(wr_no) as min_wr_no from  $table where bo_table = '$tb_id' ";
    $row = sql_fetch($sql);
    // 가장 작은 번호에 1을 빼서 넘겨줌
    return (int)($row['min_wr_no'] + 1);
}
...
function get_paging_bootst($write_pages, $cur_page, $total_page, $url, $add=""){
	...
	$str .="<li><a href='#'>3</a></li>";
	$str .="<li><a href='#'>4</a></li>";
	$str .="<li><a href='#'>5</a></li>";
...
function get_paging_bootst($write_pages, $cur_page, $total_page, $url, $add=""){
    ...
    if ($cur_page > 1) {
        $str .= '<li><a href="'.$url.'1'.$add.'" aria-label="Previous"><span aria-hidden=true>&laquo;</span></a></li>'.PHP_EOL;
    }
        ...
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                $str .= '<li><a href="'.$url.$k.$add.'" >'.$k.'</a></li>'.PHP_EOL;
				 ...
				 $str .="<li class='active' ><a href='list.php?&amp;page=$k'>".$k."</a></li>";
		
		}
    }

    if ($total_page > $end_page) $str .= '<li><a href="'.$url.($end_page+1).$add.'" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>'.PHP_EOL;

    if ($cur_page < $total_page) {
        $str .= '<li><a href="'.$url.$total_page.$add.'" class="end"><span aria-hidden="true">&raquo;</span></a></li>'.PHP_EOL;
    }

#    $str .="<li class='active' ><a href='/member/index.php?&amp;page=1'>1</a></li>";
#    $str .="<li class='' ><a href='/member/index.php?&amp;page=1'>2</a></li>";
#    $str .="<li><a class='next' href='/member/index.php?&amp;page=11'>&gt;</a></li>";
#    $str .="<li><a class='end' href='/member/index.php?&amp;page=422' title='마지막'>&gt;&gt;</a></li>";
	 $str .="</ul>";


    if ($str)
		 return "<div class='pagination pagination-centered'>{$str}</div>";
	else
        return "";
}
