<?
    ...
    if ($member[mb_level] < 2)
    {
        if ($member[mb_id])
            ...
            alert("접근 권한이 없습니다.\\n\\회원이시라면 관리자에게 문의해 보십시오.", "/member/login.php?wr_id=$wr_id{$qstr}&url=".urlencode("/board/list.php?bo_table=$bo_table"));
    }
	...
	else
	{
		...
		$sql = " select distinct wr_id from $write_table where  bo_table = '$bo_table'";
		$result = sql_query($sql);
		$total_count = mysqli_num_rows($result);
...
//	$write_table = 'psj_board';
	...
	$i = 0;
		...
		else
		{
			...
			//$sql = " select * from $write_table where wr_is_comment = 0 $sql_order limit $from_record,$board[bo_page_rows] "; // $board[bo_page_rows]
			$sql = " select * from $write_table where wr_subject != '' and  bo_table = '$bo_table' order by wr_id DESC limit 0,15"; // $board[bo_page_rows]
